...
### 12.6 菜单管理的本质：前端先有页面，后端再配菜单

理解了动态路由后，你可能会问：**后端新增一条菜单记录，前端就自动多出一个页面吗？**

答案是：**不是**。前端得先有那个 `.vue` 文件，后端菜单的 `component` 字段才能找到它。
...
① 前端开发人员编写新的 .vue 页面
        ↓
② 将 .vue 文件放到 views/ 对应目录下
        ↓
③ 后端/管理员在数据库插入一条菜单记录
  （component 字段指向那个 .vue 文件路径）
        ↓
④ 用户刷新页面，看到新菜单，点击能正常打开
```
...
| 操作 | 说明 | 举例 |
|------|------|------|
| 调整排序 | 修改 sort 字序 | 把"关于"从第3位调到第1位 |
| 调整父子关系 | 修改 parent_id | 把"菜单管理"从"系统管理"移到"首页"下 |
| 修改名称/图标 | 修改显示信息 | 把"用户管理"改成"账号管理" |
| 显示/隐藏菜单 | 控制菜单是否展示 | 暂时隐藏"角色管理"，不删数据 |
| 新增菜单 | 前提是前端页面已开发好 | 新增"日志管理"（对应 `system/log/index.vue` 已存在） |

#### 总结

```
前端开发决定 → "系统有哪些功能"（写 .vue 页面）
菜单管理决定 → "这些功能怎么组织和展示"（调整结构、排序、显隐）
```
